...
// 프론트엔드(localhost:3000)에서 백엔드 API를 호출할 수 있도록 CORS 정책을 설정하는 클래스
//
// 현재 프로젝트는 프론트와 백엔드가 서로 다른 Origin에서 실행되기 때문에
// 브라우저가 차단하지 않도록 허용 Origin, 허용 메서드, 쿠키/인증 포함 여부 등을 명시적으로 설정
@Configuration
class WebCorsConfig(
    // application.yml 에서 CORS 허용 Origin 목록을 문자열로 받는다.
    //
    // 예
    ...
    fun corsConfigurationSource(): CorsConfigurationSource {
        val configuration = CorsConfiguration().apply {
            allowedOrigins = parseOrigins().toList()
    ...
    //
    // 공백은 제거하고, 비어 있는 값은 제외해서 Spring CORS 설정에 바로 넘길 수 있는 형태로 만든다.
