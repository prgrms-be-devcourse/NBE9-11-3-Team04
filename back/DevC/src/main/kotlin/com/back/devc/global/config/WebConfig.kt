...
//
// 현재 댓글 첨부파일은 서버의 uploads 디렉터리에 저장되므로
...
class WebConfig : WebMvcConfigurer {
    ...
    override fun addResourceHandlers(registry: ResourceHandlerRegistry) {
        // 실행 중인 프로젝트 루트 기준으로 uploads 폴더 경로를 찾는다.
        val uploadPath = Paths.get(System.getProperty("user.dir"), "uploads")

        // Spring ResourceHandler가 읽을 수 있도록 file URI 형태로 변환한다.
        val uploadDir = uploadPath.toUri().toString().let { path ->
            if (path.endsWith("/")) path else "$path/"
        }

        // /uploads/** 로 들어온 요청을 실제 uploads 폴더에서 찾도록 연결한다.
        // 이 설정이 있어야 댓글 첨부파일 URL을 프론트에서 그대로 사용할 수 있다.
