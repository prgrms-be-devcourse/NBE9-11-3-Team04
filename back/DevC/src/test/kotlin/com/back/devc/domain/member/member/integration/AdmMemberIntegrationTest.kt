package com.back.devc.domain.member.member.integration
...
class AdmMemberIntegrationTest {
    ...
    fun adminListsMembers() {
        ...
        mockMvc.perform(
            ...
            .andExpect(MockMvcResultMatchers.jsonPath("$.data.content[0].email").value("adm-list@test.com"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.data.content[0].nickname").value("adm-list"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.data.content[0].postCount").value(0))
            .andExpect(MockMvcResultMatchers.jsonPath("$.data.content[0].commentCount").value(0))
    }
    ...
    fun adminReadsDetailAndUpdatesStatus() {
        ...
        mockMvc.perform(
            MockMvcRequestBuilders.get("/api/admin/members/{userId}", userId)
                .header(HttpHeaders.AUTHORIZATION, "Bearer $token"),
        )
            .andExpect(MockMvcResultMatchers.status().isOk)
            .andExpect(MockMvcResultMatchers.jsonPath("$.code").value("ADMIN_MEMBER_200_DETAIL_SUCCESS"))
        ...
        mockMvc.perform(
            MockMvcRequestBuilders.patch("/api/admin/members/{userId}/status", userId)
                .header(HttpHeaders.AUTHORIZATION, "Bearer $token")
            ...
            .andExpect(MockMvcResultMatchers.status().isOk)
            .andExpect(MockMvcResultMatchers.jsonPath("$.code").value("ADMIN_MEMBER_200_STATUS_UPDATE_SUCCESS"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.data.userId").value(userId))
